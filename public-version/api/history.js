...
// separate from /api/latest matters especially on the Public version,
...
export default async function handler(req, res) {
  ...
  //
  // Fix: page through with .range() until a page comes back short
  // (meaning we've reached the real end), accumulating everything server-side
  // before responding.
  //
  // PAGE_SIZE assumes the project default of 1000 but works correctly even
  // if that dashboard setting is later raised or lowered.
  ...
  try {
    while (true) {
      ...
      if (data.length < PAGE_SIZE || rows.length >= HARD_CAP) {
        break;
      }
